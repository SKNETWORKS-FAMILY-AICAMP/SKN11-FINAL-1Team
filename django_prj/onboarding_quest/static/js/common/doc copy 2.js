...
function renderDocTable(docs) {
  ...
  docs.forEach(doc => {
    ...
    tr.innerHTML = `
      ...
      <td>
      ${doc.department_id === CURRENT_DEPARTMENT_ID
            ? `<input type="checkbox" class="doc-checkbox" data-doc-id="${doc.docs_id}">`
            : `<span style="color:#999;">-</span>`}
      </td>
      <td>
        <a href="http://localhost:8001/api/docs/documents/download/${doc.docs_id}">
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("bulk-delete-btn")?.addEventListener("click", async () => {
    ...
    for (const cb of selected) {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          console.warn(`❌ 삭제 실패 (HTTP ${res.status}):`, errText);
    ...
    let msg = `✅ ${successCount}개 문서가 삭제되었습니다.`;
    if (failCount > 0) {
      msg += `\n⚠️ ${failCount}개 문서는 삭제 실패 (권한 또는 오류).`;
    }
